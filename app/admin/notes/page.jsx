...
// Reusing same logic as Syllabus for notes, simply pointing to /api/notes
export default function AdminNotesPage() {
    ...
    const fetchNotes = async () => {
        try {
            ...
            if (result.success) setNotesList(result.data);
        } catch (err) {
    ...
    const handleUpload = async (e) => {
        ...
        try {
            ...
            // 2. Create Note Record on Express Backend
            const sysRes = await apiPost('/notes', {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            setNotesList(notesList.filter(s => s._id !== id));
        } catch (err) {
        ...
        <div className="p-6 max-w-7xl mx-auto page-enter">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Upload files for students to download</p>
                </div>
                <button onClick={() => setShowForm(true)} className="btn-primary">
                    <PlusIcon className="w-5 h-5" /> Upload Note
                </button>
            </div>
                ...
                <table className="w-full data-table">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr><td colSpan="4" className="text-center py-8 text-slate-500">No notes uploaded yet.</td></tr>
                        ) : notesList.map((item) => (
                            <tr key={item._id}>
                                <td>
                                ...
                                <td>
                                    <div className="flex items-center justify-end gap-2">
                                        <a href={item.fileUrl} target="_blank" rel="noreferrer" className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg" title="Download">
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                        ...
                        <form onSubmit={handleUpload} className="space-y-4">
                            <div>
                                ...
                                <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="form-input" placeholder="e.g. Chapter 4 Motion Review" />
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" required value={subject} onChange={e => setSubject(e.target.value)} className="form-input" placeholder="e.g. Physics" />
                            </div>
                            <div>
                                <label className="form-label">File</label>
                                <input type="file" required onChange={e => setFile(e.target.files[0])} className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition-colors" />
                            </div>
